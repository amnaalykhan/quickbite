...
function getDB() {
  ...
  const dbPath = path.join(app.getPath('userData'), 'quickbite.db');
...
function startServer(port, customerMenuPath) {
  return new Promise((resolve, reject) => {
    ...
    expressApp.use(express.static(customerMenuPath));
    ...
    expressApp.get('/api/orders', (req, res) => {
      try {
        ...
        orders.forEach(o => {
          o.items = database.prepare('SELECT * FROM order_items WHERE order_id=?').all(o.id);
        });
        res.json(orders);
      } catch(e) { res.status(500).json({ error: e.message }); }
    });
    ...
    expressApp.get('/api/settings', (req, res) => {
      try {
        const database = getDB();
        res.json(database.prepare('SELECT * FROM restaurant WHERE id=1').get());
      } catch(e) { res.status(500).json({ error: e.message }); }
    });
    ...
    expressApp.put('/api/settings', (req, res) => {
      try {
        ...
        database.prepare(
          'UPDATE restaurant SET name=?, tagline=?, currency=?, table_count=?, wifi_password=? WHERE id=1'
        ).run(name, tagline, currency || '₹', table_count || 10, wifi_password || '');
        res.json({ success: true });
      } catch(e) { res.status(500).json({ error: e.message }); }
    });

    // Get full menu for management
    ...
    expressApp.put('/api/menu/items/:id', (req, res) => {
      try {
        ...
        database.prepare(
          'UPDATE menu_items SET name=?,description=?,price=?,is_veg=?,is_bestseller=?,is_available=? WHERE id=?'
    ...
    expressApp.delete('/api/menu/items/:id', (req, res) => {
      try {
        const database = getDB();
        database.prepare('DELETE FROM menu_items WHERE id=?').run(req.params.id);
        res.json({ success: true });
      } catch(e) { res.status(500).json({ error: e.message }); }
    });
    ...
    expressApp.patch('/api/menu/items/:id/toggle', (req, res) => {
      try {
        const database = getDB();
        database.prepare('UPDATE menu_items SET is_available = 1 - is_available WHERE id=?').run(req.params.id);
        res.json({ success: true });
      } catch(e) { res.status(500).json({ error: e.message }); }
    });
    ...
    expressApp.delete('/api/menu/categories/:id', (req, res) => {
      try {
        const database = getDB();
        database.prepare('DELETE FROM menu_items WHERE category_id=?').run(req.params.id);
    ...
    httpServer.listen(port, '0.0.0.0', () => {
      console.log(`QuickBite server on http://0.0.0.0:${port}`);
      resolve(io);
    });
